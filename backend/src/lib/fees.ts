/**
 * Frais de plateforme Mastercota (payés par l'organisateur).
 * Les contributions contributeur → orga sont hors de ce module (Wave direct / suivi).
 */
...
/** Paliers : 1–15 → 2000 ; 16–35 → 4000 ; 36–60 → 10_000 */
...
}): { ok: true; newDeadline: string; fee: number } | { ok: false; error: string } {
...
export function deadlineFromDuration(durationDays: number, from = new Date()): string {
  return formatDateOnly(addDays(toDateOnly(from)!, durationDays));
}

/** @deprecated Ancien modèle ~3 % contributeur — ne plus utiliser pour les contributions. */
...
/** Identité : plus de surcoût contributeur. */
export function feesFromNet(netInput: number): FeeQuote {
  const net = Math.round(Number(netInput));
  if (!Number.isFinite(net) || net <= 0) throw new Error("Montant invalide");
